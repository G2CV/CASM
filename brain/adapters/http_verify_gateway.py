...
@dataclass
class HttpVerifyGateway:
    tool_path: str
    timeout_ms: int = 10000

    def run(self, payload: dict) -> dict:
        """Execute the http-verify tool and return raw JSON output.

        Notes:
            This adapter intentionally returns raw JSON because the HTTP tool
            owns its own evidence/SARIF emission logic.
        """
